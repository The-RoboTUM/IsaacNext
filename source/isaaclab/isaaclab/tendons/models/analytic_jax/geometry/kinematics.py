...
class TendonCoordinates(NamedTuple):
    ...
    joint_angles_signed: jnp.ndarray
    thetas: jnp.ndarray
    theta_hats: jnp.ndarray
    qs: jnp.ndarray
    qhats: jnp.ndarray


def compute_tendon_coordinates(joint_angles: jnp.ndarray, tendon_data: TendonDataJIT) -> TendonCoordinates:
    ...
    thetas = jnp.empty_like(tendon_data.tendon_offsets_theta)
    theta_ids = [
        tids.I_THETA_GST_3,
        tids.I_THETA_GST_4,
        tids.I_THETA_GST_5,
    ...
    thetas = thetas.at[:, theta_ids].set(joint_angles_signed[:, joint_ids] + tendon_data.tendon_offsets_theta[:, theta_ids])

    qs = jnp.empty_like(tendon_data.tendon_offsets_q_theta)
    q_ids = [
        tids.I_Q_GST_3,
        tids.I_Q_GST_4,
        tids.I_Q_GST_5,
    ...
    qs = qs.at[:, q_ids].set(thetas[:, theta_for_q_ids] + tendon_data.tendon_offsets_q_theta[:, q_ids])

    theta_hats = -thetas + 2 * jnp.pi

    qhats = jnp.empty_like(tendon_data.tendon_offsets_qhat_thetahat)
    qhats = qhats.at[:, tids.I_QHAT_EDT2_6].set(
