...
class SpringEnergyTensors(NamedTuple):
    energy: jnp.ndarray
    gst_energy: jnp.ndarray
    dft_energy: jnp.ndarray
    kft_energy: jnp.ndarray
    edt1_energy: jnp.ndarray
    edt2_energy: jnp.ndarray
    gst_active: jnp.ndarray
    dft_active: jnp.ndarray
    kft_active: jnp.ndarray
    edt1_active: jnp.ndarray
    edt2_active: jnp.ndarray


@dataclass
class SpringEnergyOutput:
    energy: jnp.ndarray
    per_tendon_energy: dict[str, jnp.ndarray]
    active_masks: dict[str, jnp.ndarray]


def compute_spring_energy_from_delta_lengths(
    gst_delta_l: jnp.ndarray,
    dft_delta_l: jnp.ndarray,
    kft_delta_l: jnp.ndarray,
    edt1_delta_l: jnp.ndarray,
    edt2_delta_l: jnp.ndarray,
    ...
    total_energy = (
        jnp.where(gst_active, gst_energy, jnp.zeros_like(gst_energy)).sum()
        + jnp.where(dft_active, dft_energy, jnp.zeros_like(dft_energy)).sum()
        + jnp.where(kft_active, kft_energy, jnp.zeros_like(kft_energy)).sum()
        + jnp.where(edt1_active, edt1_energy, jnp.zeros_like(edt1_energy)).sum()
        + jnp.where(edt2_active, edt2_energy, jnp.zeros_like(edt2_energy)).sum()
    )
...
class SpringEnergyModel:
    ...
    def energy_from_delta_tuple(
        self,
        deltas: tuple[jnp.ndarray, jnp.ndarray, jnp.ndarray, jnp.ndarray, jnp.ndarray],
    ) -> SpringEnergyOutput:
    ...
    def energy_from_deltas(self, deltas: dict[str, jnp.ndarray]) -> SpringEnergyOutput:
