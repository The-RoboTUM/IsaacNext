...
def _safe_sqrt(value: torch.Tensor) -> torch.Tensor:
    return torch.sqrt(torch.clamp(value, min=1.0e-12))


@torch.jit.script
def _safe_acos(value: torch.Tensor) -> torch.Tensor:
    return torch.acos(torch.clamp(value, -1.0 + 1.0e-6, 1.0 - 1.0e-6))
